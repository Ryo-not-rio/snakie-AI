...
def mutate(weight1,weight2):
    ...
    mutechoice=[0.8,0.5,0.07,0.1,0.07,0.07,0.05,0.3,0.02,0.01]
        ...
        if instcount<=creaturenum-1:
            if count<framelimit:
                if changed:
                    ...
                    if maximum<max(finish) and generation%1000000==0 or generation<changelim:
                        maximum=max(finish)
                        muteindex=0
                    if min(finish)<minimum and generation%100000000==0 or generation<changelim:
                        minimum=min(finish)   
                        muteindex=0 
